...
/* Room — full-screen custom video meeting */
#view-room { position:fixed; inset:0; z-index:100; display:flex; flex-direction:column; background:#000; }

/* Video grid */
#videoGrid {
  flex:1; display:flex; flex-wrap:wrap; align-items:center; justify-content:center;
  gap:4px; padding:4px; overflow:hidden; background:#111;
}
.vtile {
...
#callControls {
  ...
  background:rgba(0,0,0,.85); backdrop-filter:blur(10px); flex-shrink:0;
}
.ctrl-btn {
  width:54px; height:54px; border-radius:50%; border:none; cursor:pointer;
  font-size:22px; background:#2a2f3e; color:#fff; transition:background .15s;
  display:flex; align-items:center; justify-content:center;
}
.ctrl-btn.muted  { background:#b91c1c; }
...
<div id="view-room" class="hidden">
  ...
  <div id="callControls">
    <button id="micBtn"  class="ctrl-btn" onclick="toggleMic()" title="Toggle mic">🎙️</button>
    <button id="camBtn"  class="ctrl-btn" onclick="toggleCam()" title="Toggle camera">📹</button>
...
function toggleMic() {
  ...
  if (activeMeeting.self.audioEnabled) {
    ...
    btn.textContent = '🎙️'; btn.classList.remove('muted');
  }
}
