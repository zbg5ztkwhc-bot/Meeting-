...
async function startMeeting(authToken, code) {
  ...
  try {
    // Ensure the web component is registered before we need it
    ...
    // Show room container FIRST and wait one animation frame so the browser
    // computes real layout dimensions before the component initialises
    document.getElementById('view-lobby').classList.add('hidden');
    document.getElementById('view-room').classList.remove('hidden');
    hideLoading();
    await new Promise(r => requestAnimationFrame(r));

    // Assign meeting — the <rtk-meeting> component takes it from here:
    // shows setup screen, lets user configure camera/mic, then joins on click
    ...
    hideLoading();
    // Put lobby back and show error (room might have been revealed)
    document.getElementById('view-lobby').classList.remove('hidden');
    document.getElementById('view-room').classList.add('hidden');
