...
<!-- RealtimeKit UI Kit (registers <rtk-meeting> web component) -->
<script type="module">
...
#rtkEl     { width:100%; height:100%; display:block; }
...
<div id="view-room" class="hidden">
  <rtk-meeting id="rtkEl" show-setup-screen="true"></rtk-meeting>
</div>
...
// ─── iOS Safari: inject playsinline into shadow DOM videos ───────────────────
function fixIOSVideo(rootEl) {
  function walk(el) {
...
async function startMeeting(authToken, code) {
  ...
  try {
    ...
    // Show room container first and wait one rAF so the browser computes
    // real layout dimensions before the component initialises
    document.getElementById('view-lobby').classList.add('hidden');
    document.getElementById('view-room').classList.remove('hidden');
    hideLoading();
    await new Promise(r => requestAnimationFrame(r));

    const rtkEl = document.getElementById('rtkEl');
    rtkEl.meeting = meeting;

    // When the room is joined, repeatedly inject playsinline into the
    // component's shadow DOM videos — iOS Safari refuses to play any
    // video element that lacks this attribute, causing a black screen
    meeting.self.on('roomJoined', () => {
      const delays = [200, 600, 1200, 2500, 5000];
      delays.forEach(ms => setTimeout(() => fixIOSVideo(rtkEl), ms));
    });
