...
async function startMeeting(authToken, code) {
  ...
  try {
    ...
    // Show room FIRST so the component renders into a visible container with real dimensions
    document.getElementById('roomCodeDisplay').textContent = code;
    document.getElementById('view-lobby').classList.add('hidden');
    document.getElementById('view-room').classList.remove('hidden');
    hideLoading();

    // Ensure <rtk-meeting> custom element is registered before use
    await customElements.whenDefined('rtk-meeting');

    // Recreate the element fresh — the old one was in a display:none container
    // and may have initialised with zero dimensions
    const oldEl = document.getElementById('rtkEl');
    const rtkEl = document.createElement('rtk-meeting');
    rtkEl.id = 'rtkEl';
    rtkEl.setAttribute('show-setup-screen', 'true');
    rtkEl.style.cssText = 'width:100%;height:100%;display:block;';
    oldEl.parentNode.replaceChild(rtkEl, oldEl);

    // Assign meeting to the fresh, visible element
