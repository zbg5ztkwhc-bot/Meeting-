...
<div id="view-room" class="hidden">
  ...
  <div id="setupScreen" class="hidden">
    ...
    <div class="setup-label">Camera ready — tap Join to enter</div>
    <button id="joinNowBtn" onclick="joinNow()">Join Meeting</button>
  </div>
...
let localStream   = null;  // raw getUserMedia stream for preview / self-tile
...
async function startMeeting(authToken, code) {
  ...
  try {
    // 1. Get local camera — we know getUserMedia works on iOS (setup screen proved it)
    ...
    document.getElementById('view-room').classList.add('hidden');
    document.getElementById('view-lobby').classList.remove('hidden');
    showLobbyErr('Error: ' + err.message);
  }
}
...
async function joinNow() {
  ...
  try {
    ...
    // ── Self tile: use the live localStream we already captured ──────────────
    ...
    // Update self video if SDK provides a different track later
    ...
    console.error('[RTK] joinNow error:', err);
    hideLoading();
    document.getElementById('joinNowBtn').disabled = false;
    showLobbyErr('Join error: ' + err.message);
  }
}
